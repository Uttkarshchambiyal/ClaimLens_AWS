...
def _desktop_flow(page: Page, base_url: str, artifacts: Path | None) -> None:
    ...
    page.get_by_role("heading", name="Review dashboard").wait_for()
    page.get_by_role("heading", name="See the workload at a glance").wait_for()
    page.get_by_role("heading", name="One transparent review path").wait_for()
    _assert_page_basics(page)
    _screenshot(page, artifacts, "desktop-dashboard.png")
    page.get_by_role("button", name="Review queue").click()
    page.get_by_role("heading", name="Review queue").first.wait_for()
    _assert_page_basics(page)
    _screenshot(page, artifacts, "desktop-queue.png")
    page.get_by_role("button", name="Use dark theme").click()
    page.get_by_role("button", name="Use light theme").wait_for()
    page.wait_for_timeout(250)
    claim_theme = page.locator(".claim-link").first.evaluate(
        "element => ({color: getComputedStyle(element).color, root: document.documentElement.className, foreground: getComputedStyle(document.documentElement).getPropertyValue('--foreground')})"
    )
    claim_color = claim_theme["color"]
    if claim_color == "rgb(18, 32, 35)":
        raise AssertionError(f"Dark-theme claim links use the light-theme foreground: {claim_theme}")
    _assert_page_basics(page)
    _screenshot(page, artifacts, "desktop-queue-dark.png")
    ...
    page.get_by_role("heading", name="Review insights").wait_for()
    _assert_page_basics(page)
    _screenshot(page, artifacts, "desktop-insights.png")
    page.get_by_role("tab", name=re.compile(r"Findings")).click()
...
def _mobile_flow(page: Page, base_url: str, artifacts: Path | None) -> None:
    ...
    page.get_by_role("heading", name="See the workload at a glance").wait_for()
    _assert_page_basics(page)
    _screenshot(page, artifacts, "mobile-dashboard.png")
    page.get_by_role("button", name="Review queue").click()
    page.get_by_role("heading", name="Review queue").first.wait_for()
    _assert_page_basics(page)
