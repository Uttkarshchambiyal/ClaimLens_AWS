...
| Tenant isolation | `test_tenant_isolation_blocks_cross_tenant_claim_access` | Cognito/API authorization verified for one tenant; cross-tenant denial pending |
...
- Playwright exercises the current mock landing, login, signup, queue, claim review, disposition mutation, audit activity, evidence viewer, and upload dialog in Chromium at 1536x960 and 390x844. It also checks page errors, console errors, duplicate DOM IDs, image alt text, and viewport-level overflow. Nine current screenshots were manually inspected. Production Cognito signup, confirmation, login, refresh, logout, private tenant-claim issuance, and a protected API read are verified; cross-tenant HTTP denial and signed-source rendering remain unverified.
...
- AWS CLI and SAM CLI are installed and authenticated. The `claimlens` stack is `UPDATE_COMPLETE` in `ap-south-1`; Amplify, Cognito, API Gateway, Lambda, Step Functions, S3, DynamoDB and CloudWatch are deployed. Hosted Cognito login and a protected tenant-scoped queue request pass in Chromium. Textract returns `SubscriptionRequiredException`, and Bedrock returns `Operation not allowed`, so extraction/model execution remains blocked by AWS account activation.
...
- Provision a second retained test tenant and verify cross-tenant HTTP denial against an actual record. Self-service signup, confirmation, private tenant-claim issuance, login, refresh, logout, and one protected queue request are verified; the temporary signup acceptance user was deleted.
